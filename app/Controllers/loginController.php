...
class LoginController
{
    ...
    public static function validate()
    {
        ...
        $user = new User;
        //check if login a locked out login
        ...
        //check if it's a valid login and set session and redirect
    ...
    private function loginFailed($email)
    {
        ...
        $user = new User;
        $time = (int)shell_exec("date '+%s'");
        if ($failed_login_details = $user->getFailedlogin($email))  //get last lgon details
            extract($failed_login_details, EXTR_SKIP);
        else
            Controller::redirect("/Login/view", ["loginErr" => 'Incorrect username or password', "login" => true]); //if there's email or password doesn't match redirect to login

        //check if the user has exceeded attempts
        if (($failed_login_count >= $bad_login_limit) && ($time - $first_failed_login < $lockout_time)) {
            ...
            $user->updateFailedLogin($email, $failed_login_count, $first_failed_login); //update failed login details
